...
namespace Лаб_4
{
    class main_menu
    {
        ...
        public static void Main_menu()
        {
            ...
            while (true)
            {
                ...
                AnsiConsole.MarkupLine("[purple]6.[/] Змінити тип контейнера");
                ...
                AnsiConsole.MarkupLine("[darkgreen]11.[/] Завантажити контейнер з файлу");
    ...
    : new LinkedListContainer<Product>();
        ...
        {
            AnsiConsole.Markup("[grey]Введіть шлях до файлу для збереження: [/]");
            string filePath = Console.ReadLine();
            try
            {
                products.SaveToFile(filePath);
                AnsiConsole.MarkupLine("[green]Контейнер успішно збережено![/]");
            }
            catch (Exception ex)
            {
                AnsiConsole.MarkupLine($"[red]Помилка: {ex.Message}[/]");
        ...
        {
            AnsiConsole.Markup("[grey]Введіть шлях до файлу для завантаження: [/]");
            string filePath = Console.ReadLine();
            try
            {
                products.LoadFromFile(filePath);
                AnsiConsole.MarkupLine("[green]Контейнер успішно завантажено![/]");
            }
            catch (Exception ex)
            {
                AnsiConsole.MarkupLine($"[red]Помилка: {ex.Message}[/]");
